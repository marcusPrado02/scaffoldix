...
and makes them available to Handlebars templates. _Transforms_ automatically
...
```handlebars
// Generated file:
{{entityNameKebabCase}}.service.ts export class
{{entityNamePascalCase}}Service { readonly tableName = "{{entityNameSnakeCase}}s"; readonly
routePath = "/api/{{entityNameKebabCase}}s"; }
```
...
To rename _files and directories_, use rename rules when calling `generate`:
