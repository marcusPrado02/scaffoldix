...
async function saveState(stateFile: string, state: ProjectState): Promise<void> {
  ...
  await fs.writeFile(tmp, JSON.stringify({ ...state, updatedAt: new Date().toISOString() }, null, 2), "utf-8");
...
Examples:
  ...
  scaffoldix rollback --dry-run`,
    ...
    .action(
      async (options: { target: string; yes: boolean; dryRun: boolean }) => {
        const ux = getCliUx();
        ...
        try {
          ...
          const generations: GenerationReport[] = state.generations ?? (state.lastGeneration ? [state.lastGeneration] : []);
